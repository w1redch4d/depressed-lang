...
private:
  ...
  std::shared_ptr<Stmt> declaration() {
    try {
      if (match(GROUP)) return classDeclaration();
      if (match(DEPRESSION)) return function("function");
      if (match(PAIN)) return varDeclaration();
  ...
  std::shared_ptr<Stmt> statement() {
    if (match(ITER)) return forStatement();
    if (match(IF)) return ifStatement();
    if (match(CRY)) return printStatement();
  ...
  std::shared_ptr<Expr> primary() {
    if (match(NO)) return std::make_shared<Literal>(false);
    if (match(YES)) return std::make_shared<Literal>(true);
